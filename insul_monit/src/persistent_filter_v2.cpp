...
public:
    PersistentVoxelFilter(ros::NodeHandle &nh) : nh_(nh) {
        nh_.param("N", N_, 3);
        nh_.param("M", M_, 2);  // Default: A voxel must be occupied in at least M=2 frames
...
private:
    ...
    int N_;  // Number of frames in rolling buffer
    int M_;  // Minimum frames a voxel must be occupied in N
    ...
    void cloudCallback(const PointCloudT::ConstPtr &cloud) {
        ...
        // Populate the current frame's voxel map
        for (const auto &point : cloud->points) {
            VoxelKey voxel = { 
                static_cast<int>(point.x / voxel_size_),
        ...
        // If we exceed the buffer size, remove the oldest frame
        ...
        // Update occupancy count for current frame
        for (const auto &kv : current_frame_voxels) {
            voxel_occupancy_[kv.first]++;
        }

        // Generate filtered output cloud
        ...
        for (const auto &point : cloud->points) {
            ...
            // Pass voxels that have been occupied in at least M frames
            if (voxel_occupancy_[voxel] >= M_) {
